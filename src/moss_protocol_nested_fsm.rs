...
pub(crate) fn extract_packet<'a>(
    ...
    unit_id.map(|unit_id| MossPacket {
        unit_id,
        hits: data_fsm::extract_hits(bytes).unwrap_or_else(|| Vec::with_capacity(0)),
    })
}
...
pub mod data_fsm {
    ...
    #[inline]
    pub fn add_data0(moss_hits: &mut Vec<MossHit>, data0: u8, region: u8) {
    ...
    #[inline]
    pub fn add_data2(moss_hits: &mut [MossHit], data2: u8) {
